...
from projectname_sdk.core.control import IpGeolocationControl
...
class IpGeolocationContext:
    def __init__(self, ctxmap=None, basectx=None):
        ...
        # Ctrl
        self.ctrl = IpGeolocationControl()
        ...
        if isinstance(sp, IpGeolocationSpec):
            self.spec = sp
        elif basectx is not None:
        ...
        if isinstance(r, IpGeolocationResult):
            self.result = r
        elif basectx is not None:
        ...
        if isinstance(rp, IpGeolocationResponse):
            self.response = rp
        elif basectx is not None:
    ...
    def make_error(self, code, msg):
        return IpGeolocationError(code, msg, self)
