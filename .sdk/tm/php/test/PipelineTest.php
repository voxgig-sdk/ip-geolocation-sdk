...
// IpGeolocation SDK pipeline test
//
...
{
    private static function utility(): IpGeolocationUtility
    {
        return new IpGeolocationUtility();
    }
    ...
    private static function ctx(array $over = []): IpGeolocationContext
    {
        $utility = $over['utility'] ?? self::utility();
        $client = $over['client'] ?? new PlClient(['base' => 'http://h']);
        $ctx = new IpGeolocationContext([
            'client' => $client,
            'utility' => $utility,
        ], null);
        $ctx->op = new IpGeolocationOperation(['name' => 'load', 'entity' => 'x']);
        foreach ($over as $k => $v) {
    ...
    {
        return ($err instanceof IpGeolocationError) ? $err->sdk_code : '';
    }
    ...
    public function test_make_point_rejects_a_disallowed_operation(): void
    {
        ...
        $ctx->op = new IpGeolocationOperation(['name' => 'nope', 'entity' => 'x']);
        [$point, $err] = IpGeolocationMakePoint::call($ctx);
    ...
    public function test_make_point_rejects_an_operation_with_no_endpoints(): void
    {
        ...
        $ctx->op = new IpGeolocationOperation(['name' => 'load', 'entity' => 'x', 'points' => []]);
        [$point, $err] = IpGeolocationMakePoint::call($ctx);
        $this->assertNull($point);
    ...
    public function test_make_point_returns_the_single_point(): void
    {
        ...
        $ctx = self::ctx(['options' => ['allow' => ['op' => 'load,list,create,update,remove']]]);
        $ctx->op = new IpGeolocationOperation(['name' => 'load', 'entity' => 'x', 'points' => [$point]]);
        [$got, $err] = IpGeolocationMakePoint::call($ctx);
        $this->assertNull($err);
    ...
    {
        $preset = new IpGeolocationSpec(['method' => 'GET']);
        $ctx = self::ctx();
        $ctx->out['spec'] = $preset;
        [$got, $err] = IpGeolocationMakeSpec::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    public function test_make_response_guards_missing_spec_response_result(): void
    {
        $ctx = self::ctx([
            'spec' => null,
            'response' => new IpGeolocationResponse([]),
            'result' => new IpGeolocationResult([]),
        ]);
        [, $err] = IpGeolocationMakeResponse::call($ctx);
        $this->assertSame('response_no_spec', self::code($err));

        $ctx = self::ctx([
            'spec' => new IpGeolocationSpec([]),
            'response' => null,
            'result' => new IpGeolocationResult([]),
        ]);
        [, $err] = IpGeolocationMakeResponse::call($ctx);
    ...
    {
        $ctx = self::ctx([
            'spec' => new IpGeolocationSpec(['step' => 's']),
            'response' => new IpGeolocationResponse(self::resp(404, null, ['x-a' => '1'])),
            'result' => new IpGeolocationResult([]),
        ]);
        [, $err] = IpGeolocationMakeResponse::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_response_2xx_parses_the_body_and_marks_ok(): void
    {
        $ctx = self::ctx([
            'spec' => new IpGeolocationSpec(['step' => 's']),
            'response' => new IpGeolocationResponse(self::resp(200, ['v' => 1])),
            'result' => new IpGeolocationResult([]),
        ]);
        [, $err] = IpGeolocationMakeResponse::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_response_records_to_ctrl_explain_when_explain_is_on(): void
    {
        $ctx = self::ctx([
            'spec' => new IpGeolocationSpec(['step' => 's']),
            'response' => new IpGeolocationResponse(self::resp(200, ['v' => 2])),
            'result' => new IpGeolocationResult([]),
        ]);
        $ctx->ctrl->explain = ['on' => true];
        IpGeolocationMakeResponse::call($ctx);
    ...
    {
        $preset = new IpGeolocationResponse(self::resp(299));
        $ctx = self::ctx([
            'spec' => new IpGeolocationSpec([]),
            'response' => new IpGeolocationResponse([]),
            'result' => new IpGeolocationResult([]),
        ...
        [$got, $err] = IpGeolocationMakeResponse::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    {
        $ctx = self::ctx(['spec' => null, 'result' => new IpGeolocationResult([])]);
        [, $err] = IpGeolocationMakeResult::call($ctx);
        $this->assertSame('result_no_spec', self::code($err));

        $ctx = self::ctx(['spec' => new IpGeolocationSpec([]), 'result' => null]);
        [, $err] = IpGeolocationMakeResult::call($ctx);
    ...
    public function test_make_result_list_op_wraps_resdata_into_entity_instances(): void
    {
        ...
        $ctx = self::ctx([
            'entity' => $entity,
            'spec' => new IpGeolocationSpec(['step' => 's']),
            'result' => new IpGeolocationResult(['ok' => true, 'resdata' => [['a' => 1], ['a' => 2]]]),
        ]);
        $ctx->op = new IpGeolocationOperation(['name' => 'list', 'entity' => 'x']);
        [$result, $err] = IpGeolocationMakeResult::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_result_empty_list_yields_empty_resdata(): void
    {
        ...
        $ctx = self::ctx([
            'entity' => $entity,
            'spec' => new IpGeolocationSpec(['step' => 's']),
            'result' => new IpGeolocationResult(['ok' => true, 'resdata' => []]),
        ]);
        $ctx->op = new IpGeolocationOperation(['name' => 'list', 'entity' => 'x']);
        [$result, $err] = IpGeolocationMakeResult::call($ctx);
        $this->assertNull($err);
    ...
    {
        $preset = new IpGeolocationResult(['ok' => true]);
        $ctx = self::ctx([
            'spec' => new IpGeolocationSpec([]),
            'result' => new IpGeolocationResult([]),
        ]);
        $ctx->out['result'] = $preset;
        [$got, $err] = IpGeolocationMakeResult::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    public function test_make_request_guards_a_missing_spec(): void
    {
        ...
        [, $err] = IpGeolocationMakeRequest::call($ctx);
    ...
    {
        $utility = self::utility();
        $boom = new IpGeolocationError('boom', 'boom');
        $utility->fetcher = function (IpGeolocationContext $_c, string $_u, array $_f) use ($boom): array {
            return [null, $boom];
        };
        $ctx = self::ctx([
            'utility' => $utility,
            'spec' => new IpGeolocationSpec(['step' => 's', 'method' => 'GET', 'base' => 'http://h', 'parts' => ['a']]),
        ]);
        [$response, $err] = IpGeolocationMakeRequest::call($ctx);
        $this->assertNull($err);
    ...
    {
        $utility = self::utility();
        $utility->fetcher = function (IpGeolocationContext $_c, string $_u, array $_f): array {
            return [null, null];
        };
        $ctx = self::ctx([
            'utility' => $utility,
    ...
    public function test_make_request_a_normal_transport_response_is_wrapped(): void
    {
        ...
        [$response, $err] = IpGeolocationMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertInstanceOf(IpGeolocationResponse::class, $response);
    ...
    public function test_make_request_records_the_fetchdef_to_ctrl_explain(): void
    {
        ...
        $ctx->ctrl->explain = ['on' => true];
        IpGeolocationMakeRequest::call($ctx);
    ...
    {
        $utility = self::utility();
        $utility->make_fetch_def = function (IpGeolocationContext $c): array {
            return [null, $c->make_error('fetchdef_boom', 'boom')];
        };
        $ctx = self::ctx([
            'utility' => $utility,
            'spec' => new IpGeolocationSpec(['step' => 's', 'method' => 'GET']),
        ]);
        [$response, $err] = IpGeolocationMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertNotNull($response->err);
    ...
    {
        $preset = new IpGeolocationResponse(self::resp(201));
        $ctx = self::ctx(['spec' => new IpGeolocationSpec([])]);
        $ctx->out['request'] = $preset;
        [$got, $err] = IpGeolocationMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    {
        $ctx = self::ctx(['spec' => null]);
        [, $err] = IpGeolocationMakeFetchDef::call($ctx);
    ...
    public function test_make_fetch_def_serialises_body_and_inits_missing_result(): void
    {
        ...
        [$fetchdef, $err] = IpGeolocationMakeFetchDef::call($ctx);
        $this->assertNull($err);
    ...
    {
        $ctx = self::ctx(['result' => new IpGeolocationResult(['ok' => true, 'resdata' => 42])]);
        $this->assertSame(42, IpGeolocationDone::call($ctx));
    }

    public function test_done_raises_the_error_when_not_ok(): void
    {
        $ctx = self::ctx(['result' => new IpGeolocationResult(['ok' => false])]);
        $this->expectException(IpGeolocationError::class);
        IpGeolocationDone::call($ctx);
    }

    public function test_make_error_returns_resdata_when_ctrl_throw_is_false(): void
    {
        $ctx = self::ctx(['result' => new IpGeolocationResult(['ok' => false, 'resdata' => 'fallback'])]);
        $ctx->ctrl->throw_err = false;
        $this->assertSame('fallback', IpGeolocationMakeError::call($ctx, null));
    }
    ...
    public function test_make_error_records_to_ctrl_explain(): void
    {
        ...
        $ctx->ctrl->throw_err = false;
        $ctx->ctrl->explain = ['on' => true];
        IpGeolocationMakeError::call($ctx, null);
    ...
    public function test_feature_add_appends_in_call_order(): void
    {
        ...
        $a = new IpGeolocationBaseFeature();
        $b = new IpGeolocationBaseFeature();
        IpGeolocationFeatureAdd::call($ctx, $a);
        IpGeolocationFeatureAdd::call($ctx, $b);
        $this->assertSame([$a, $b], $client->features);
    }

    private static function named_feature(string $name): IpGeolocationBaseFeature
    {
        $f = new IpGeolocationBaseFeature();
        $f->name = $name;
        return $f;
    }
    ...
    public function test_feature_add_ordering_before_after_replace(): void
    {
        ...
        IpGeolocationFeatureAdd::call($ctx, $miss);
    ...
    private static function auth_ctx(array $options, ?array $headers): IpGeolocationContext
    {
        $client = new PlClient($options);
        return self::ctx([
            'client' => $client,
            'spec' => $headers === null ? null : new IpGeolocationSpec(['headers' => $headers]),
        ]);
    }
    ...
    public function test_prepare_auth_guards_a_missing_spec(): void
    {
        ...
        [, $err] = IpGeolocationPrepareAuth::call($ctx);
    ...
    public function test_prepare_auth_a_raw_apikey_goes_in_as_is(): void
    {
        ...
        IpGeolocationPrepareAuth::call($ctx);
    ...
    {
        $ctx = self::ctx([
            'response' => new IpGeolocationResponse(['headers' => null]),
            'result' => new IpGeolocationResult([]),
        ]);
        IpGeolocationResultHeaders::call($ctx);
    ...
    public function test_result_body_skips_parsing_when_the_body_is_absent(): void
    {
        ...
        IpGeolocationResultBody::call($ctx);
