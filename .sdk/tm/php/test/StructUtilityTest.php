...
// IpGeolocation SDK struct utility test
//
...
class StructUtilityTest extends TestCase
{
    ...
    {
        if (self::$run === null) {
            $runner = IpGeolocationOmni::makeStructRunner(
        ...
        $this->runsection(
            ...
            function ($input) {
                ...
                if (is_object($store) && $haspath && Struct::undef() !== $val) {
                    ...
                    if (json_encode(IpGeolocationOmni::structfix($now, Struct::undef()))
                        !== json_encode(IpGeolocationOmni::structfix($val, Struct::undef()))) {
