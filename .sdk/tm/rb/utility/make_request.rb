...
module IpGeolocationUtilities
  MakeRequest = ->(ctx) {
    ...
    spec = ctx.spec
    utility = ctx.utility
    response = IpGeolocationResponse.new({})
    ...
    if fetch_err
      ...
      response = IpGeolocationResponse.new({ "err" => ctx.make_error("request_no_response", "response: undefined") })
    elsif fetched.is_a?(Hash)
      response = IpGeolocationResponse.new(fetched)
    else
