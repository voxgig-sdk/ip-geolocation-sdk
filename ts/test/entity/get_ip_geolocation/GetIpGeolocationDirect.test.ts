...
describe('GetIpGeolocationDirect', async () => {
  ...
  test('direct-exists', async () => {
    const sdk = new IpGeolocationSDK({
  ...
  test('direct-load-get_ip_geolocation', async (t: any) => {
    if (liveScenariosActive()) { t.skip('Covered by live operation scenarios'); return }
    const setup = directSetup({ id: 'direct01' })
    if (maybeSkipControl(t, 'direct', 'direct-load-get_ip_geolocation', setup.live)) return
...
function directSetup(mockres?: any) {
  ...
  if (live) {
    ...
    let idmap: any = env['IP_GEOLOCATION_TEST_GET_IP_GEOLOCATION_ENTID']
