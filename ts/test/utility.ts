...
function envOverride(m: Record<string, any>) {
  if (
    'TRUE' === process.env.IPGEOLOCATION_TEST_LIVE ||
    'TRUE' === process.env.IPGEOLOCATION_TEST_OVERRIDE
  ) {
  ...
  m.IPGEOLOCATION_TEST_EXPLAIN = process.env.IPGEOLOCATION_TEST_EXPLAIN || m.IPGEOLOCATION_TEST_EXPLAIN
