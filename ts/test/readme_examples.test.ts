...
//     (\`new IpGeolocationSDK(...)\` / \`IpGeolocationSDK.test(...)\` become
//     \`IpGeolocationSDK.test({ entity: ... })\`), transpiled to JS, and run
...
const SDK_NAME = 'IpGeolocationSDK'
...
const TEST_SEED = {"entity":{"get_ip_geolocation":{"test01":{"id":"test01"}}}}
...
// rewrite real client construction (\`new IpGeolocationSDK(...)\` and any
// \`IpGeolocationSDK.test(...)\`) to a seeded test-mode client, and give
...
function rewriteForRun(code: string): string {
  ...
  out = out.replace(/new\s+IpGeolocationSDK\s*\([^)]*\)/g, () => SEEDED_CTOR)
  out = out.replace(/IpGeolocationSDK\.test\s*\([^)]*\)/g, () => SEEDED_CTOR)
