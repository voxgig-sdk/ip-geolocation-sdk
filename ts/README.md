...
The TypeScript SDK for the IpGeolocation API — a type-safe, entity-oriented client with full async/await support.

The API is exposed as capitalised, semantic **Entities** — e.g.
`client.GetIpGeolocation()` — each with a small set of operations (`load`)
...
| `tester(testopts?, sdkopts?)` | `IpGeolocationSDK` | Create a test-mode client instance. |

#### Static methods

| Method | Returns | Description |
| --- | --- | --- |
| `IpGeolocationSDK.test(testopts?, sdkopts?)` | `IpGeolocationSDK` | Create a test-mode client. |
...
| `client` | `client(): IpGeolocationSDK` | Return the parent SDK client. |
...
```ts
const get_ip_geolocation = await client.GetIpGeolocation().load()
```
...
ip-geolocation/
├── src/
│   ├── IpGeolocationSDK.ts        # Main SDK class
