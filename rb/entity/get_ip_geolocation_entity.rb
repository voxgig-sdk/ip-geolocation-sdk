...
class GetIpGeolocationEntity
  ...
  def make
    opts = @_entopts.dup
    GetIpGeolocationEntity.new(@_client, opts)
  end

  def data_set(args)
    if args
      @_data = IpGeolocationHelpers.to_map(VoxgigStruct.clone(args)) || {}
      @_utility.feature_hook.call(@_entctx, "SetData")
    end
  end

  # @return [GetIpGeolocation, Hash] the current GetIpGeolocation data
  def data_get
  ...
  def match_set(args)
    if args
      @_match = IpGeolocationHelpers.to_map(VoxgigStruct.clone(args)) || {}
      @_utility.feature_hook.call(@_entctx, "SetMatch")
    end
  end

  # @return [Hash] the current match filter (any subset of GetIpGeolocation fields)
  def match_get
  ...
  #
  # @param reqmatch [GetIpGeolocationLoadMatch, Hash, nil] match criteria (id/query fields);
  #   optional — an entity with no id-like key loads with no match (nil is treated
  #   as an empty match, so client.GetIpGeolocation.load works with no args).
  # @param ctrl [Object, nil] optional per-call control
  # @return [GetIpGeolocation, Hash] the loaded GetIpGeolocation; raises IpGeolocationError on failure
    ...
    _run_op(ctx) do
      if ctx.result
        ...
        if ctx.result.resdata
          @_data = IpGeolocationHelpers.to_map(VoxgigStruct.clone(ctx.result.resdata)) || {}
        end
      end
    end
  end
