...
require_relative 'IpGeolocation_types'
...
class IpGeolocationSDK
  ...
  def initialize(options = {})
    ...
    feature_opts = IpGeolocationHelpers.to_map(VoxgigStruct.getprop(@options, "feature"))
    if feature_opts
      featureorder = VoxgigStruct.getpath(@options, "__derived__.featureorder")
      if featureorder.is_a?(Array)
        featureorder.each do |fname|
          fopts = IpGeolocationHelpers.to_map(feature_opts[fname])
          if fopts && fopts["active"] == true
            utility.feature_add.call(@_rootctx, IpGeolocationFeatures.make_feature(fname))
          end
        end
      end
    end
  ...
  def get_utility
    IpGeolocationUtility.copy(@_utility)
  end
  ...
  def prepare(fetchargs = {})
    ...
    ctrl = IpGeolocationHelpers.to_map(VoxgigStruct.getprop(fetchargs, "ctrl")) || {}
    ...
    params = IpGeolocationHelpers.to_map(VoxgigStruct.getprop(fetchargs, "params")) || {}
    query = IpGeolocationHelpers.to_map(VoxgigStruct.getprop(fetchargs, "query")) || {}
  ...
  def raw_request(fetchargs = {})
    ...
    if fetched.is_a?(Hash)
      status = IpGeolocationHelpers.to_int(VoxgigStruct.getprop(fetched, "status"))
  ...
  def graphql(query, variables = nil, ctrl = nil)
    ...
    if errors.is_a?(Array) && !errors.empty?
      ...
      res["ok"] = false
      res["err"] = IpGeolocationError.new(
        "graphql_error", "IpGeolocationSDK: graphql: #{msg}")
      res["graphql"] = errors
    end
  ...
  def self.test(testopts = nil, sdkopts = nil)
    ...
    sdk = IpGeolocationSDK.new(sdkopts)
    sdk.mode = "test"
    sdk
  end
end
