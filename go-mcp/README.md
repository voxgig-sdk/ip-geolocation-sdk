...
[MCP](https://modelcontextprotocol.io) server exposing the IpGeolocation SDK as
two agent tools — `ip-geolocation_list` and `ip-geolocation_load` — built on the
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/ip-geolocation-mcp)
make build

# 2. Provide credentials via the environment
export IP_GEOLOCATION_APIKEY=sk_live_xxx

# 3a. Install into Claude Code over stdio (most common)
claude mcp add --scope user ip-geolocation \
  -- /absolute/path/to/ip-geolocation-mcp -transport stdio

# 3b. …or run a shared HTTP server instead
./ip-geolocation-mcp -transport http -addr :8080
```

Tool-call arguments (what an agent sends):

```jsonc
// ip-geolocation_list: first page of records
{ "entity": "get_ip_geolocation" }
{ "entity": "get_ip_geolocation", "query": { } }

// ip-geolocation_load: one record by id
{ "entity": "get_ip_geolocation", "query": { "id": 1 } }
```
...
1. **Build** the server from this `go-mcp/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/ip-geolocation-mcp
   ```
...
3. **Install it into Claude Code** (stdio transport):
   ...
   ```sh
   claude mcp add --scope user ip-geolocation \
     -- "$PWD"/dist/*/ip-geolocation-mcp -transport stdio
   ```

4. **Restart Claude Code.** The `ip-geolocation_list` and `ip-geolocation_load` tools now appear
   in new sessions. Ask the agent to *"list get_ip_geolocation using ip-geolocation"*
   and it calls `ip-geolocation_list` with `{"entity":"get_ip_geolocation"}`.
...
```sh
export IP_GEOLOCATION_APIKEY=sk_live_xxx            # API key
export IP_GEOLOCATION_BASE=https://api.example.com  # optional: override the API base URL
```
...
### Call the `ip-geolocation_list` tool
...
### Call the `ip-geolocation_load` tool
...
| Tool | Args | Returns |
|------|------|---------|
| `ip-geolocation_list` | `entity` (required), `query` (optional map) | First page of records as JSON |
| `ip-geolocation_load` | `entity` (required), `query` = `{id:N}` | Single record as JSON |
...
| Variable | Purpose |
|----------|---------|
| `IP_GEOLOCATION_APIKEY` | API key sent with every request. |
| `IP_GEOLOCATION_BASE` | Optional override of the API base URL. |
...
```sh
./ip-geolocation-mcp -transport http -addr :18080 &
...
curl -sN -X POST http://localhost:18080 \
  ...
  -d '{"jsonrpc":"2.0","id":2,"method":"tools/call","params":{"name":"ip-geolocation_load","arguments":{"entity":"get_ip_geolocation","query":{"id":1}}}}'
```
