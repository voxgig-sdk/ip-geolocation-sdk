...
// IpGeolocation SDK feature test
//
...
class FtCtrl extends IpGeolocationControl
{
...
class FtClient
{
    ...
    private IpGeolocationUtility $utility;

    public function __construct(IpGeolocationUtility $utility, array $options)
    {
    ...
    public function get_utility(): IpGeolocationUtility
    {
        return $this->utility;
    }
}
...
class FtRecorder
{
    ...
    {
        return function (IpGeolocationContext $_ctx, string $url, array $fetchdef): array {
...
class FtHarness
{
    ...
    public IpGeolocationContext $rootctx;
    ...
    public function __construct(
        ...
        $utility->fetcher = $server ?? function (IpGeolocationContext $_ctx, string $_url, array $fetchdef): array {
        ...
        foreach ($features as $fspec) {
            ...
            $f = IpGeolocationConfig::make_feature($name);
    ...
    private static function buildUrl(IpGeolocationSpec $spec): string
    {
        $keys = [];
    ...
    public function op(array $o = []): array
    {
        ...
        if (($ctx->out['point'] ?? null) instanceof IpGeolocationError) {
        ...
        if (is_array($fetched)) {
            $ctx->response = new IpGeolocationResponse($fetched);
        }
    ...
    private function fail(IpGeolocationContext $ctx, mixed $err): array
    {
...
class FeatureTest extends TestCase
{
    ...
    public function test_retry_retries_a_transport_error_until_exhausted(): void
    {
        ...
        $clock = new FtClock();
        $n = 0;
        $server = function (IpGeolocationContext $ctx, string $_url, array $_fd) use (&$n): array {
            $n++;
