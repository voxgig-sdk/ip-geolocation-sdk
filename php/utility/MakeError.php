...
class IpGeolocationMakeError
{
    public static function call(?IpGeolocationContext $ctx, mixed $err): mixed
    {
        if ($ctx === null) {
            require_once __DIR__ . '/../core/Context.php';
            $ctx = new IpGeolocationContext([], null);
        }
        $op = $ctx->op ?? new IpGeolocationOperation([]);
        $opname = $op->name;
        if ($opname === '' || $opname === '_') {
            $opname = 'unknown operation';
        }

        $result = $ctx->result ?? new IpGeolocationResult([]);
        $result->ok = false;
        ...
        $errmsg = ($err instanceof IpGeolocationError) ? $err->msg : (string)$err;
        ...
        $sdk_err = new IpGeolocationError('', $msg, $ctx);
        $sdk_err->result = ($ctx->utility->clean)($ctx, $result);
        $sdk_err->spec = ($ctx->utility->clean)($ctx, $spec);
        if ($err instanceof IpGeolocationError) {
