...
class IpGeolocationSDK
{
    ...
    public function __construct(array $options = [])
    {
        ...
        $utility = new IpGeolocationUtility();
        ...
        $feature_opts = IpGeolocationHelpers::to_map(Struct::getprop($this->options, "feature"));
        if ($feature_opts) {
            $featureorder = Struct::getpath($this->options, "__derived__.featureorder");
            if (is_array($featureorder)) {
                foreach ($featureorder as $fname) {
                    $fopts = IpGeolocationHelpers::to_map($feature_opts[$fname] ?? null);
                    if ($fopts && isset($fopts["active"]) && $fopts["active"] === true) {
                        ...
                        if (!IpGeolocationFeatures::has_feature($fname)) {
                        ...
                        ($utility->feature_add)($this->_rootctx, IpGeolocationFeatures::make_feature($fname));
                    }
                }
            }
        }
    ...
    {
        return IpGeolocationUtility::copy($this->_utility);
    }
    ...
    public function prepare(array $fetchargs = []): mixed
    {
        ...
        $ctrl = IpGeolocationHelpers::to_map(Struct::getprop($fetchargs, "ctrl")) ?? [];
        ...
        $params = IpGeolocationHelpers::to_map(Struct::getprop($fetchargs, "params")) ?? [];
        $query = IpGeolocationHelpers::to_map(Struct::getprop($fetchargs, "query")) ?? [];
    ...
    private function op_denied(string $op): array
    {
        ...
        return [
            "ok" => false,
            "err" => new IpGeolocationError($op . "_allow",
                "IpGeolocationSDK: " . $op . ": operation not allowed by" .
    ...
    private function raw_request(array $fetchargs = []): mixed
    {
        ...
        if (is_array($fetched)) {
            $status = IpGeolocationHelpers::to_int(Struct::getprop($fetched, "status"));
    ...
    public function graphql(string $query, ?array $variables = null, ?array $ctrl = null): mixed
    {
        ...
        if (is_array($errors) && 0 < count($errors)) {
            ...
            $res["ok"] = false;
            $res["err"] = new IpGeolocationError("graphql_error",
                "IpGeolocationSDK: graphql: " . $msg);
            $res["graphql"] = $errors;
        }
    ...
    public function GetIpGeolocation($data = null)
    {
        ...
        if ($data === null) {
            if ($this->_get_ip_geolocation === null) {
                $this->_get_ip_geolocation = new GetIpGeolocationEntity($this, null);
            }
            return $this->_get_ip_geolocation;
        }
        return new GetIpGeolocationEntity($this, $data);
    }
    ...
    public static function test(?array $testopts = null, ?array $sdkopts = null): self
    {
        ...
        $sdk = new IpGeolocationSDK($sdkopts);
        $sdk->mode = "test";
        return $sdk;
    }
}
