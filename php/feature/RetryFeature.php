...
// IpGeolocation SDK retry feature
...
class IpGeolocationRetryFeature extends IpGeolocationBaseFeature
{
    ...
    public function init(IpGeolocationContext $ctx, array $options): void
    {
        ...
        $utility->fetcher = function (IpGeolocationContext $ctx2, string $url, array $fetchdef) use ($inner): array {
            return $this->_with_retry($ctx2, $url, $fetchdef, $inner);
        };
    }

    private function _with_retry(IpGeolocationContext $ctx, string $url, array $fetchdef, callable $inner): array
    {
        $opts = $this->options ?? [];
    ...
    private function _track(IpGeolocationContext $_ctx, int $attempt, mixed $res, mixed $err, float $wait): void
    {
        $client = $this->client;
