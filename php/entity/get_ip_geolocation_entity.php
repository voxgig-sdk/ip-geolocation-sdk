...
// IpGeolocation SDK GetIpGeolocation entity
...
class GetIpGeolocationEntity
{
    ...
    {
        $opts = $this->_entopts;
        return new GetIpGeolocationEntity($this->_client, $opts);
    }

    /**
     * @param GetIpGeolocation|array $args GetIpGeolocation data (assoc-array) to store.
     */
    public function data_set($args): void
    {
        if ($args) {
            $this->_data = IpGeolocationHelpers::to_map(Struct::clone($args)) ?? [];
            ($this->_utility->feature_hook)($this->_entctx, "SetData");
        }
    }

    /**
     * @return GetIpGeolocation|array The current GetIpGeolocation data as an assoc-array.
     */
    ...
    /**
     * @param array $args Match filter (any subset of GetIpGeolocation fields).
     */
    public function match_set($args): void
    {
        if ($args) {
            $this->_match = IpGeolocationHelpers::to_map(Struct::clone($args)) ?? [];
            ($this->_utility->feature_hook)($this->_entctx, "SetMatch");
        }
    }

    /**
     * @return array The current match filter (any subset of GetIpGeolocation fields).
     */
    ...
    /**
     * Load a single GetIpGeolocation.
     *
     * @param GetIpGeolocationLoadMatch|array|null $reqmatch Match criteria (id/query
     *   fields) as an assoc-array; a typed GetIpGeolocationLoadMatch names the shape.
     * @param mixed $ctrl Optional per-call control overrides.
     * @return GetIpGeolocation|array The loaded GetIpGeolocation as an assoc-array at the
     *   SDK boundary; throws IpGeolocationError on failure (item-5 convention).
     */
        ...
        return $this->_run_op($ctx, function () use ($ctx) {
            if ($ctx->result) {
                ...
                if ($ctx->result->resdata) {
                    $this->_data = IpGeolocationHelpers::to_map(Struct::clone($ctx->result->resdata)) ?? [];
                }
            }
        });
    }
