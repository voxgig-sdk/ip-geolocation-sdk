...
class TestGetIpGeolocationDirect:

    def test_should_direct_load_get_ip_geolocation(self):
        setup = _get_ip_geolocation_direct_setup({"id": "direct01"})
        _skip, _reason = runner.is_control_skipped("direct", "direct-load-get_ip_geolocation", "live" if setup["live"] else "unit")
        if _skip:
...
def _get_ip_geolocation_direct_setup(mockres):
