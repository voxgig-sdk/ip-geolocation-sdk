...
class IpGeolocationTestRunner:
    ...
    def load_env_local():
        ...
        for line in content.splitlines():
            ...
            IpGeolocationTestRunner._env[key] = val

    @staticmethod
    def getenv(key):
        val = IpGeolocationTestRunner._env.get(key)
        if val is not None:
            return val
    ...
    @staticmethod
    def env_override(m):
        live = IpGeolocationTestRunner.getenv("IP_GEOLOCATION_TEST_LIVE")
        override = IpGeolocationTestRunner.getenv("IP_GEOLOCATION_TEST_OVERRIDE")

        if live == "TRUE" or override == "TRUE":
            for key in list(m.keys()):
                envval = IpGeolocationTestRunner.getenv(key)
        ...
        explain = IpGeolocationTestRunner.getenv("IP_GEOLOCATION_TEST_EXPLAIN")
        if explain is not None and explain != "":
            m["IP_GEOLOCATION_TEST_EXPLAIN"] = explain
    ...
    def load_test_control():
        ...
        """
        if IpGeolocationTestRunner._test_control is not None:
            return IpGeolocationTestRunner._test_control
        ctrl_path = os.path.join(os.path.dirname(__file__), "sdk-test-control.json")
        try:
            with open(ctrl_path, "r") as f:
                IpGeolocationTestRunner._test_control = json.load(f)
        except (FileNotFoundError, IOError, ValueError):
            IpGeolocationTestRunner._test_control = {
                "version": 1,
                "test": {"skip": {
    ...
    def is_control_skipped(kind, name, mode):
        ...
        ctrl = IpGeolocationTestRunner.load_test_control()
...
def load_env_local():
    IpGeolocationTestRunner.load_env_local()


def env_override(m):
    return IpGeolocationTestRunner.env_override(m)


def entity_data(v):
    return IpGeolocationTestRunner.entity_data(v)


def entity_list_to_data(lst):
    return IpGeolocationTestRunner.entity_list_to_data(lst)


def is_control_skipped(kind, name, mode):
    return IpGeolocationTestRunner.is_control_skipped(kind, name, mode)


def load_test_control():
    return IpGeolocationTestRunner.load_test_control()


def live_client_options():
    return IpGeolocationTestRunner.live_client_options()


def live_delay_ms():
    return IpGeolocationTestRunner.live_delay_ms()
