...
local IpGeolocationSDK = {}
IpGeolocationSDK.__index = IpGeolocationSDK
...
IpGeolocationSDK._make_feature = _make_feature


function IpGeolocationSDK.new(options)
  local self = setmetatable({}, IpGeolocationSDK)
  self.mode = "live"
  self.features = {}
  self.options = nil
...
function IpGeolocationSDK:options_map()
...
function IpGeolocationSDK:get_utility()
  return Utility.copy(self._utility)
end


function IpGeolocationSDK:get_root_ctx()
  return self._rootctx
end


function IpGeolocationSDK:prepare(fetchargs)
...
function IpGeolocationSDK:direct(fetchargs)
...
-- by passing it.
function IpGeolocationSDK:_raw_request(fetchargs)
  local utility = self._utility
...
function IpGeolocationSDK:graphql(query, variables, ctrl)
  ...
  if type(errors) == "table" and 0 < #errors then
    ...
    res.ok = false
    res.err = "IpGeolocationSDK: graphql: " .. msg
    res.graphql = errors
  end
...
function IpGeolocationSDK.test(testopts, sdkopts)
  sdkopts = sdkopts or {}
  ...
  local sdk = IpGeolocationSDK.new(sdkopts)
  sdk.mode = "test"
