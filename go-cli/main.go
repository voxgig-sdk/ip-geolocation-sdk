// Command IpGeolocation-cli is a CLI + REPL for the IpGeolocation SDK,
...
const prompt = "ip-geolocation"

// entitiesHelp is the space-separated entity list shown by /help.
const entitiesHelp = "get_ip_geolocation"
...
func usage(out io.Writer) {
	...
	fmt.Fprintln(out, "Env:      IP_GEOLOCATION_APIKEY (api key), IP_GEOLOCATION_BASE (base url override)")
...
func run(args []string, in io.Reader, out, errOut io.Writer) int {
	...
	// Configure from the environment: IP_GEOLOCATION_APIKEY carries the API key and
	// IP_GEOLOCATION_BASE optionally overrides the API base URL (e.g. production).
	// Both injectable by a secrets vault. Unset -> nil config defaults.
	var opts map[string]any
	if apikey := os.Getenv("IP_GEOLOCATION_APIKEY"); apikey != "" {
		opts = map[string]any{"apikey": apikey}
	}
	if base := os.Getenv("IP_GEOLOCATION_BASE"); base != "" {
		if opts == nil {
			opts = map[string]any{}
		}
		opts["base"] = base
	}
	client := sdk.NewIpGeolocationSDK(opts)
