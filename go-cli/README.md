...
boru-driven command-line client **and** interactive REPL for the IpGeolocation
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/ip-geolocation-cli)
make build

# 2. See usage (words, entities, env vars)
./ip-geolocation-cli --help

# 3. Provide credentials once, via the environment
export IP_GEOLOCATION_APIKEY=sk_live_xxx

# 4. Each command line is ONE boru expression, run against the API:
./ip-geolocation-cli load 1 get_ip_geolocation            # {id:1} shorthand
./ip-geolocation-cli load '{id:1}' get_ip_geolocation       # explicit match map

# 5. Override the API base URL for a single call
IP_GEOLOCATION_BASE=https://api.example.com ./ip-geolocation-cli load 1 get_ip_geolocation

# 6. No arguments -> interactive REPL
./ip-geolocation-cli
ip-geolocation> load 1 get_ip_geolocation
ip-geolocation> /quit
```
...
1. **Build the binary.** From this `go-cli/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/ip-geolocation-cli
   ```
...
3. **Run a query.** Evaluate an boru expression against the API (or run with no
   ...
   ```sh
   ./dist/*/ip-geolocation-cli load 1 get_ip_geolocation
   ```
...
```sh
./ip-geolocation-cli load 1 get_ip_geolocation          # scalar shorthand for {id:1}
./ip-geolocation-cli load '{id:1}' get_ip_geolocation     # explicit match map
```
...
./ip-geolocation-cli load 1 get_ip_geolocation
```
...
Run with no arguments to open a REPL (prompt `ip-geolocation>`). Each line is
...
- `<entity>` is a bareword, auto-quoted as an boru atom (e.g. `get_ip_geolocation`).
...
| Target | Result |
|--------|--------|
| `make build` | Native binary at `dist/<os>-<arch>/ip-geolocation-cli`. |
...
(`./ip-geolocation-cli <expr>`) and interactively (the REPL), and expressions compose the
